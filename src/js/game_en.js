var messages = {
    ...
    blocks: {
        ...
        requestCount_tip : 'Count of requests made by travelers inside the elevator and store in its "memory". Check that the count of requests is 1 or more before calling \'earliest request\'.'
    },
    ...
    help: {
        ...
        4: '<p>Since both persons get on the same floor, they are going to be pressing the buttons immediately, for floor 5 and floor 3.<p></p> You cannot just go to the floors as they get pressed. You need to \'save\' them in a queue. When the elevator door closes, you can see if there are any requests (Hint: \'request count\' is greater than 0) then send the elevator to that floor.</p><p>That way, each time the elevator door is closed, you will be processing the \'requests\' one at a time till there are none left and all travelers have been dispatched to their destination!</p>'
    }
};
